...
void unpack_marshal_call_send(char* buffer, int buflen, Details brokerDetails, Config brokerConfig)
{
    ...
    while ((return_status = msgpack_unpack_next(&unpacked_result, buffer, buflen, &off)) == MSGPACK_UNPACK_SUCCESS) {
        ...
        if(STR_Equals(OPERATION_HDR, header_name) && val.type == MSGPACK_OBJECT_STR) {
            char* ptrStr = (char*) malloc(sizeof(char) * (size_t)val.via.str.size +1);
            ...
            call_marshelResponse_send(message_id, ptrOperation, brokerDetails.address, brokerDetails.port, brokerConfig.verbose,(char**)params);
        }
    } // finished unpacking.
