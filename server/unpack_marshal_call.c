        ...
        if(STR_Equals( "op", header_name) && val.type == MSGPACK_OBJECT_STR) {
            ...
            for(int i = 0; i < val.via.array.size;i++) {
                ...
                if(param_type == MSGPACK_OBJECT_STR) {
                	//param is a char*
                    char* str = malloc(sizeof(char) * param.via.str.size);
                    memset(str, '\0', param.via.str.size);
            ...
            call_marshelResponse_send(message_id, operation, broker_address, broker_port, verbose,(void**)params);
        }
        return_status = msgpack_unpack_next(&unpacked_result, buffer, buflen, &off);
    } // finished unpacking.
