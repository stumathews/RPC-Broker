...
void setupCmd(int argc, char* argv[])
{
    ...
    struct Argument* portNumber = 			CMD_CreateNewArgument("bp", "broker-port <number>", "Set the broker port that the client will connect to", true,true,setBrokerPortNumber);
    struct Argument* brokerAddress = 		CMD_CreateNewArgument("ba","broker-address <address>","Set the broker address that the client will connect to",true,true,setBrokerAddress);
    ...
    struct Argument* ourAddress = 			CMD_CreateNewArgument("oa","our-address <address>","Set our address broker will contact us on",true,true,setOurAddress);
